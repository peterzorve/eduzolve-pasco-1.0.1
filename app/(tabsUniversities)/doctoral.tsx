...
export default function DoctoralDegree() {
  const router = useRouter();
  const colorScheme = useColorScheme();
  const {universityInformation } = useLocalSearchParams();
  const universityInfo = JSON.parse(universityInformation)
  const bachelorsPrograms = degreePrograms.filter( (university) => (university.name === universityInfo?.name) &&  (university.category === "phd") );
    ...
    <View style={{ flex: 1 }}>
      <ScrollView style={{  width: "100%", alignSelf: "center", marginTop: 50 }} showsVerticalScrollIndicator={false}> 
          ...
          <View style={{ width: "90%", alignSelf: "center",  }} >
            <TitleAndDescription title={ "Doctoral programs"} titleFontSize={26} titleColor={colorScheme === "dark" ? "white" : "black"} />
