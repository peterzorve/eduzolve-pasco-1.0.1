...
export default function DiplomaDegrees() {
  const router = useRouter();
  const colorScheme = useColorScheme();
  const {universityInformation } = useLocalSearchParams();
  const universityInfo = JSON.parse(universityInformation)
  const bachelorsPrograms = degreePrograms.filter( (university) => (university.name === universityInfo?.name) &&  (university.category === "diploma") );
    ...
    <View style={{ flex: 1 }}>
      <ScrollView style={{  width: "100%", alignSelf: "center", marginTop: 50 }} showsVerticalScrollIndicator={false}> 
          ...
          <View style={{ width: "90%", alignSelf: "center",  }} >
            <TitleAndDescription title={ "Diploma programs"} titleFontSize={26} titleColor={colorScheme === "dark" ? "white" : "black"} />
              ...
              {bachelorsPrograms && bachelorsPrograms.map((programs, index) => (
                ...
                style={[{ 
                  ...
                  <Text style={{   color: colorScheme === "dark" ? "gray" : "gray",   fontSize: 10,   fontFamily: "Kanit",   marginBottom: 10, }}>
                    ...
                    <Text style={{   color: colorScheme === "dark" ? "gold" : "black", textDecorationLine: "underline" }}>
                      {programs?.web_pages} 
                    </Text>
                  </Text>
